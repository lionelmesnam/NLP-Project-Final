...
## 📖 Giới thiệu (Overview)
Dự án này tập trung vào việc áp dụng và so sánh hiệu năng của các mô hình ngôn ngữ lớn (Transformer-based models) để giải quyết bài toán **Phân tích cảm xúc văn bản tiếng Việt** (Sentiment Analysis).
...
* **Notebooks (Thực nghiệm mô hình):**
    * `baseline.ipynb`: Các mô hình máy học cơ bản (SVM, Naive Bayes...) làm cơ sở so sánh.
    ...
    * `phobert.ipynb` & `phobert-no-other.ipynb`: Huấn luyện và tinh chỉnh mô hình **PhoBERT** (State-of-the-art cho tiếng Việt).
* **Application:**
